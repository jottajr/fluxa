...
export function ProfileSwitcher() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 rounded-md border border-[var(--border-subtle)] px-2.5 py-1.5 text-sm hover:bg-[var(--accent)]/10"
      >
        <span>{activeProfile.icon}</span>
        <span className="hidden max-w-[120px] truncate font-medium text-[var(--foreground)] sm:inline">
          {activeProfile.name}
        </span>
        <svg
          viewBox="0 0 20 20"
      ...
      {open && (
        <div className="absolute left-0 top-full z-30 mt-1 w-64 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface)] py-1 shadow-lg">
          <p className="px-3 py-1.5 text-xs font-medium uppercase tracking-wide text-[var(--text-tertiary)]">
            Trocar perfil
          </p>
          ...
          <div className="mt-1 border-t border-[var(--border-subtle)] pt-1">
            {!showNewForm ? (
              <button
                ...
                className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-[var(--text-secondary)] hover:bg-[var(--accent)]/10"
              >
